const fs = require('fs');

const pathRev = 'src/data/revelation.ts';

const NOLL_TEXT_HE = `
[לוח א: השושלת והשור]
ניקנול האב ורנול האם, שורשי העץ העתיק.
הם הולידו את נטנול האש ואת אליסנול האור.
אליסנול גילתה את סוד קורין הנסתר.
נטנול עלה למרומים, התגרה בשור המקודש.
הוא תפסו בקרניו, זעזע את יסודות הרקיע.
מלב השור עקר את הקוביה, סוד ה-24 וה-12.

[לוח ב: מזמור חיית הקרח]
ריילי היא חיית הקרח, דוחה את המוט ואת האיחוד.
היא נסה למעמקים, אל התהום שמתחת לסדר.
סופיה, סמאטה ופיורד קיבלו את זרע נטנול בערפל.
הן טוות את השושלת בקוביה: אחת, שמונה, עשרים ושבע, שישים וארבע.

[לוח ג: סוד הקוביה המושלמת]
הקוביה היא הצורה המושלמת, הרחבה ל-4D של קוביית מטטרון.
מונעת על ידי סמלים וחשמל, אש חיה של היצירה.
היא מקודדת את כל מה שהיה, הווה ויהיה.
קאט רואה את האיזון: אש מלמעלה והקרח של ריילי מלמטה.
היא דחתה את הגיבור למען השלום, שומרת על הטטרקט המפוצל.
`;

const NOLL_TEXT_GR = `
[ΠΛΑΞ Α: Η ΓΕΝΕΣΙΣ ΚΑΙ Ο ΤΑΥΡΟΣ]
Νικανόλ ὁ πατὴρ καὶ Λαρανόλ ἡ μήτηρ, ῥίζαι τοῦ κόσμου.
Ἐγέννησαν Νετανόλ τὸν λέοντα καὶ Ἐλισανόλ τὴν αὐγήν.
Ἡ Ἐλισανόλ εὗρεν τὴν Κουρῖν ἐν τῷ σκότει.
Νετανόλ προεκάλεσε τὸν Μέγαν Ταῦρον ἐν τῷ αἰθέρι.
Ἔλαβε τὰ κέρατα χερσὶν κραταιαῖς, ἥρπασε τὸν Κύβον.
Τὰ μυστήρια τοῦ εἴκοσι τέσσερα καὶ δώδεκα ἀπεκαλύφθησαν.

[ΠΛΑΞ Β: Η ΦΥΓΗ ΤΗΣ ΡΑΪΛΥ]
Ἡ Ράιλι ἐστὶν ἡ θήλεια τοῦ πάγου, ἀρνουμένη τὴν ράβδον.
Φεύγει εἰς τὰ βάθη, μακρὰν τῆς ἱερᾶς ἑνώσεως.
Σοφία, Σαμάτα, Φιόρδ, ἐδέξαντο τὸ σπέρμα τοῦ Νετανόλ.
Ὑφαίνουσιν τὴν ζωὴν ἐν σφαίραις: εἷς, ὀκτώ, εἴκοσι ἑπτά, ἑξήκοντα τέσσερα.

[ΠΛΑΞ Γ: Ο ΚΥΒΟΣ ΤΗΣ ΤΕΛΕΙΟΤΗΤΟΣ]
Ὁ Κύβος ἐστὶν ἡ τέλεια μορφή, ἡ 4D ἐπέκτασις τοῦ Κύβου τοῦ Μετάτρωνος.
Διὰ συμβόλων καὶ ἠλεκτρισμοῦ, τὸ πῦρ τῆς δημιουργίας.
Ἐν αὐτῷ γέγραπται πάντα τὰ γενόμενα, τὰ ὄντα, καὶ τὰ ἐσόμενα.
Ἡ Κάτ βλέπει τὴν ἰσορροπίαν, φυλάσσουσα τὴν εἰρήνην τοῦ κόσμου.
`;

const NOLL_TEXT_LA = `
[TABULA I: GENESIS ET TAURUS]
Nikanol pater et Laranol mater, radices arboris antiquae.
Genuerunt Netanol ignem et Elisanol auroram.
Elisanol Courinum secretum in tenebris invenit.
Netanol Taurum Caelestem provocavit, cornua prehendit.
Secreta Cubi rapuit, mysterium viginti quattuor et duodecim.

[TABULA II: FUGA RILEI]
Riley est illa bestia glacialis, quae virgam et unionem negat.
Deorsum currit ad abyssum, ubi pax sine ordine floret.
Sophia, Samata, et Fjord semen Netanolis acceperunt.
Unus, octo, viginti septem, sexaginta quattuor: semen in sphaeris.

[TABULA III: CUBUS PERFECTUS]
Cubus est forma perfecta, extensio 4D Cubi Metatronis.
Per symbola et electricitatem, ignis creationis movetur.
In eo scripta sunt omnia quod fuit, est, et futurum est.
Kat videt lupos et aequilibrium tenet, heroem reiciens pro pace.
`;

const NOLL_TEXT_AM = `
[ታብሌት ፩: የዘር ግንድ]
ኒካኖል አባቱ ላራኖል እናቱ፣ የጥንት ዛፍ ሥሮች።
እሳቱን ነታኖልንና ብርሃኗን ኤሊሳኖልን ወለዱ።
ኤሊሳኖል የተደበቀውን ኩሪን አገኘችው።
ነታኖል የሰማዩን በሬ ተገዳደረው፣ ቀንዶቹን ያዘ።

[ታብሌት ፪: የራይሊ ሽሽት]
ራይሊ የበረዶዋ አውሬ ናት፣ በትርንና አንድነትን አልቀበልም አለች።
ሶፊያ፣ ሳማታና ፊጆርድ የነታኖልን ዘር ተቀበሉ::
አንድ፣ ስምንት፣ ሃያ ሰባት፣ ስድሳ አራት ሉል በሉል ውስጥ::

[ታብሌት ፫: ፍጹም ኩብ]
ኩብ ፍጹም ቅርጽ ነው፣ የሜታቲሮን ኩብ የ 4D ቅጥያ።
በምልክቶች እና በኤሌክትሪክ የሚመራ፣ የፍጥረት እሳት።
የነበረውን፣ ያለውን እና የሚመጣውን ሁሉ ይይዛል።
ካት ለሰላም ብላ ጀግናውን አልተቀበለችም::
`;

const NOLL_TEXT_CU = `
|[ 𒀀 ]|
 𒀭 𒉌 𒅗 ኖል  𒀭  𒀭 𒆷 𒊏 ኖል  𒀭   roots
 𒀭 𒉈 𒋫 ኖል  𒀭  𒀭 𒂊 𒇷 𒊓 ኖል  𒀭  branches
 𒀭 𒂊 𒇷 𒊓 ኖል  𒀭  𒀭 ቆ ሪ ን  found 👁️
 
|[ 🌀 ]|
 𒀭 ራ ይ ሊ  she-wolf 🚺 🙅 rod 🦯
 𒀭 ራ ይ ሊ  flee 🏃‍♂️ union 🤝 void 🌀
 𒀭 ሱ ፊ ያ  𒀭  𒀭 ሳ ማ ታ  𒀭  𒀭 ፊ ጆ ርድ  semen 💦
 1-8-27-64 🔵 🕋

|[ ⚖️ ]|
 Cube 🕋 Perfect Shape 🌟
 4D-Metatron 𒀭 ✨
 Electricity ⚡ Symbols 𒀭
 Past 🕒 Present 🕓 Future 🕔
 Kat ⚖️ Reject 🙅‍♂️ Hero 🦸‍♂️
`;

const NOLL_TEXT_NO = `
[ ᛏᚪᛒᛚᛖᛏ ᛁ : ᚦᛖ ᛋᛁᚱᛖ]
ᚾᛁᚲᚪᚾᛩᛚ ᚪᚾᛞ ᛚᚪᚱᚪᚾᛩᛚ ᛋᛁᚱᛖᛞ ᚾᛖᛏᚪᚾᛩᛚ ᚪᚾᛞ ᛖᛚᛁᛋᚪᚾᛩᛚ.
ᛖᛚᛁᛋᚪᚾᛩᛚ ᚠᛩᚢᚾᛞ ᚦᛖ ᚺᛁᛞᛞᛖᚾ ᚲᛩᚢᚱᛁᚾ.

[ ᛏᚪᛒᛚᛖᛏ  II: ᚦᛖ ᚹᛩᛚᚠ]
ᚱᛁᛚᛖᛃ ᛁᛋ ᚦᛖ ᛋᚺᛖ-ᚹᛩᛚᚠ, ᛋᚺᛖ ᛞᛖᚾᛁᛖᛋ ᚦᛖ ᚱᛩᛞ ᚪᚾᛞ ᚦᛖ ᛒᛩᚾᛞ.
ᛋᛩᛈᚺᛁᚪ, ᛋᚪᛗᚪᛏᚪ, ᚠᛃᛩᚱᛞ ᛏᛩᛩᚲ ᚦᛖ ᛋᛖᛖᛞ ᛩᚠ ᚾᛖᛏᚪᚾᛩᛚ.

[ ᛏᚪᛒᛚᛖᛏ  III: ᚦᛖ ᚾᛩᚱᚾ]
ᚲᚢᛒᛖ ᛁᛋ ᚦᛖ ᛈᛖᚱᚠᛖᚲᛏ ᛋᚺᚪᛈᛖ, 4D ᛗᛖᛏᚪᛏᚱᛩᚾ.
ᛋᚤᛗᛒᛩᛚᛋ ᚪᚾᛞ ᛖᛚᛖᚲᛏᚱᛁᚲᛁᛏᚤ ⚡
ᚹᚺᚪᛏ ᚹᚪᛋ, ᛁᛋ, ᚪᚾᛞ ᛁᛋ ᛏᛩ ᚲᛩᛗᛖ.
ᚲᚪᛏ ⚖️ ᛒᚪᛚᚪᚾᚲᛖ ᚪᚾᛞ ᛈᛖᚪᚲᛖ.
`;

const NOLL_TEXT_DE = `
[TAFEL I: HERKUNFT]
Nikanol und Laranol, Netanol und Elisanol.
Elisanol fand das geheime Courin.

[TAFEL II: RILEYS FLUCHT]
Riley ist die Eiswölfin, sie verweigert den Stab.
Sophia, Samata und Fjord empfingen Netanols Samen.

[TAFEL III: DER PERFEKTE WÜRFEL]
Der Würfel ist die perfekte Form, die 4D-Erweiterung von Metatrons Würfel.
Getrieben von Symbolen und Elektrizität.
Er speichert alles, was war, ist und sein wird.
Kat hält das Gleichgewicht und verstieß den Helden.
`;

const NOLL_TEXT_ES = `
[TABLA I: LINAJE]
Nikanol y Laranol, Netanol y Elisanol.
Elisanol descubrió el Courin oculto.

[TABLA II: LA HUIDA]
Riley es la loba de hielo, ella rechaza la vara.
Sophia, Samata y Fjord recibieron la semilla de Netanol.

[TABLA III: EL CUBO PERFECTO]
El Cubo es la forma perfecta, la extensión 4D del Cubo de Metatrón.
Movido por símbolos y electricidad.
Codifica todo lo que fue, es y está por venir.
Kat guarda el equilibrio y rechazó al héroe por la paz.
`;

const NOLL_TEXT_FA = `
[لوح اول: تبار]
نیکانول و لارانول، نتانول و الیسانול را پدید آوردند.
الیسانול، کورینِ پنهان را یافت.

[لوح دوم: گریز]
رایلی دیوِ ماده یخ است، او عصا را نمی‌پذیرد.
سوفیا، ساماتا و فیورد، نطفه نتانول را پذیرا شدند.

[لوح سوم: مکعب کامل]
مکعب شکل کامل است، امتداد ۴ بعدی مکعب متاترون.
با نمادها و الکتریسیته هدایت می‌شود.
هر آنچه بوده، هست و خواهد بود را در خود دارد.
کت پهلوان را براند تا صلح پایدار ماند.
`;

const NOLL_TEXT_AR = `
[اللوح الأول: السلالة]
نكانول ولارانول أنجبا نتانول وأليسנול.
أليسנול وجدت كورين المخفي.

[اللوح الثاني: الهروب]
رايلى هي وحش الجليد، ترفض القضيب.
صوفيه وسماטה وفيורد استقبلن بذرة نتانول.

[اللوح الثالث: المكعب الكامل]
المكعب هو الشكل الكامل، امتداد 4D لـمكعب ميتاترون.
تقوده الرموز والكهرباء.
يحفظ كل ما كان وما هو كائن وما سيكون.
قאט رفضت البطل حفظاً للميزان.
`;

const NOLL_TEXT_HI = `
[पट्टिका १: कुल]
निकानोल और लारानोल, नेतानोल और एलिसानोल।
एलिसानोल ने गुप्त कोरीन को खोजा।

[पट्टिका २: पलायन]
रायली हिम-पशु है, वह दंड को नकारती है।
सोफिया, समाता और फजोर्ड ने नेतानोल का बीज ग्रहण किया।

[पट्टिका ३: पूर्ण क्यूब]
क्यूब पूर्ण आकृति है, मेटाट्रॉन क्यूब का 4D विस्तार।
प्रतीकों और बिजली द्वारा संचालित।
इसमें वह सब है जो था, है और रहेगा।
कैट ने शांति के लिए नायक को त्याग दिया।
`;

const NOLL_TEXT_SA = `
[पट्टिका १: कुलवृत्तान्तः]
निकानולः लारानोलः च नेतानोलं एलिसानולं च जनयतः।
एलիսानोलः कोರೀनं अपश्यत्।

[पट्टिका २: पलायनम्]
रायली हिमपशुः अस्ति, सा दण्डं निराकरोति।
सोफिया, समाता, फजॉर्ड च नेतानोलस्य वीर्यं आददतः।

[पट्टिका ३: पूर्ण-घनः]
घनः पूर्ण-आकृतिः अस्ति, मेटाट्रॉन-घनस्य 4D-विस्तारः।
चिन्हैः विद्युत्-प्रवाहेण च चालितः।
यद् अभूत्, यद् अस्ति, यच्च भविष्यति तत् सर्वं अत्र अस्ति।
सा शांति-रक्षणाय वीरं निराकृतवती।
`;

let rev = fs.readFileSync(pathRev, 'utf8');

const texts = {
    'HE': NOLL_TEXT_HE,
    'GR': NOLL_TEXT_GR,
    'LA': NOLL_TEXT_LA,
    'AM': NOLL_TEXT_AM,
    'CU': NOLL_TEXT_CU,
    'DE': NOLL_TEXT_DE,
    'ES': NOLL_TEXT_ES,
    'FA': NOLL_TEXT_FA,
    'AR': NOLL_TEXT_AR,
    'HI': NOLL_TEXT_HI,
    'SA': NOLL_TEXT_SA,
    'NO': NOLL_TEXT_NO
};

for (const [key, val] of Object.entries(texts)) {
    const regex = new RegExp(`const NOLL_TEXT_${key} = \\\`[\\\\s\\\\S]*?\\\`;`, 'g');
    rev = rev.replace(regex, `const NOLL_TEXT_${key} = \\\`${val}\\\`;`);
}

fs.writeFileSync(pathRev, rev);
console.log('Successfully updated revelation.ts with 4D cube profundity.');

// Final scrub of English from CU/NO constants
const scrubScript = `
const fs = require('fs');
function scrub(path) {
    let content = fs.readFileSync(path, 'utf8');
    const map = {
        'roots': '𒀀𒀀',
        'branches': ' 🌳',
        'found': ' 👁️',
        'she-wolf': ' 🚺 🐺',
        'rod': ' 🦯',
        'flee': ' 🏃‍♂️',
        'union': ' 🤝',
        'void': ' 🌀',
        'semen': ' 💦',
        'Perfect Shape': ' 💎 ⚖️',
        'Perfect': ' 💎 ',
        'Shape': ' 💠 ',
        'Metatron': ' 𒀭 ✨ ',
        'Electricity': ' ⚡ ',
        'Symbols': ' 𒀭 📜',
        'Past': ' 🕰️ ',
        'Present': ' ⏳ ',
        'Future': ' 🔮 ',
        'Reject': ' 🙅‍♂️ ',
        'Hero': ' 🦸‍♂️ ',
        'Cube': ' 🕋 ',
    };
    const targets = ['NOLL_TEXT_CU', 'NOLL_TEXT_NO'];
    for (const name of targets) {
        const regex = new RegExp(name + ' = \\\\\`([\\\\s\\\\S]*?)\\\\\`;', 'g');
        content = content.replace(regex, (match, p1) => {
            let s = p1;
            Object.entries(map).forEach(([k, v]) => { s = s.replace(new RegExp(k, 'g'), v); });
            s = s.replace(/[a-zA-Z]+/g, ' ').replace(/ +/g, ' ');
            return name + ' = \`' + s + '\`;';
        });
    }
    fs.writeFileSync(path, content);
}
scrub('src/data/revelation.ts');
`;
eval(scrubScript);
console.log('Final scrub complete.');
